...
The data can be retrieved from the `scpdata` package using the 
`zhu2019EL()`. The dataset that is analysed in this vignette is called
`scp`. 

```{r load_data}
# adapt when the data are updated on EH
# scp <- zhu2019EL()
load("~/PhD/.localdata/scpdata/zhu2019EL.Rda")
scp <- zhu2019EL
```

The data contain 62 different `SingleCellExperiment` objects that we
...
The 60 first are the peptide to spectrum match (PSM) for each run.
PSMs were quantified and identified by MaxQuant (@Tyanova2016-yj). The
samples were acquired using a label-free protocol meaning that every 
run contains an individual sample and therefore every assay contains 
a single column. The before last assay contains the peptide 
quantifications and the last assay contains the protein 
quantifications, both also generated by MaxQuant.
...
The author ran MaxQuant with iBAQ computation enabled. Those values 
are currently stored in the `rowData` of the `proteins` assays. We 
will add an additional assay containing the iBAQ values. We can easily
create a new assay of class `SingleCellExperiment` from a data table 
using the `readSingleCellExperiment` function. We first extract the 
`rowData` and we retrieve the columns that contains the iBAQ values 
(those column names start with `iBAQ`). 
...
Next, we can supply the data table and the iBAQ quantification columns
to `readSingleCellExperiment`. We use the `Protein` information in the 
table to set the row names of the assay. Finally, we adapt to column 
names to match the column names in the `scp` data set. 

```{r create_iBAQ_SCE}
ibaqSCE <- readSingleCellExperiment(table = data.frame(ibaqData),
                                    ecol = ibaqCols,
                                    fnames = "Protein")
colnames(ibaqSCE) <- sub("iBAQ.", "", colnames(ibaqSCE))
```

We add the iBAQ assay in the `scp` data set using the `addAssay` 
function. We also preserve the one-to-one relationship between 
protein quantifications and the iBAQ quantifications using the 
`addAssayLinkOneToOne`.

```{r addAssay}
scp <- addAssay(scp, ibaqSCE, name = "proteins_iBAQ")
scp <- addAssayLinkOneToOne(scp, from = "proteins", to = "proteins_iBAQ")
```
...
rd <- longRowData(scp, i = 1:60, 
                  rowDataCols = c("Sequence", "Type", "Sample.name"))
```
...
and should be removed. This seriously limits the trust we can have in
the results.
...
sel <- names(sel)[sel]
## TODO the data became inconsistent!!
# scp <- scp[sel, , ]
...
The proteins names in the `sce` data are encoded as Ensembl ID. We 
