...
succeeded and the 4 failed assays that were selected above. A 
`QFeatures` object can be seen as a three-order array:
...
Also, we reorder the channel labels to a more conventional sequence. 
...
scp@assayLinks <- scp@assayLinks[names(tmp)]
```
...
The `AnnData` object stored as `annd` now contains the normalized data.
...
scanpy$pp$pca(annd, random_state = 1L)
scanpy$pp$neighbors(annd, n_neighbors = 30L, random_state = 5L)
scanpy$tl$leiden(annd, resolution = 0.3)
sce$leidenClust <- annd$obs$leiden
```
...
```{r}
## TODO retrieve this from scpdata
load("~/PhD/tmp/SCeptre/Schoof_et_al/results/bulk/replication/6.final.RData")
sce_sceptre <- sce_schoof
```
