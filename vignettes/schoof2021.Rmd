...
timeStart <- Sys.time() ## Time the compilation
...
The data contain 194 different `SingleCellExperiment` objects that we
...
The `proteins` assay contains the protein quantifications also 
generated by ProteomeDiscoverer and the `logNormProteins` assay 
contains the protein data processed by the authors using the SCeptre
workflow. We store the later assay as a separate `SingleCellExperiment`
object that we will use later to benchmark the replication of the 
data processing using `scp`.  Note that a `QFeatures` object can be 
seen as a three-order array: $features \times samples \times assay$. 
Hence, `QFeatures` supports three-order subsetting 
`x[rows, columns, assays]`. We here extract the last assay, and remove
it from the `QFeatures` object.

```{r}
sce_sceptre <- scp[["logNormProteins"]]
scp <- scp[, , 1:193]
```
...
succeeded and the 4 failed assays that were selected above.
...
remove channel and MS run biases. This is often also called batch 
correction. 

We extract the assay to normalize using `getWithColData`. The 
normalization method in `sceptre` expects that the sample annotation
...
and the column of the plate. The data required to reproduce this plot
is contained in the `colData`.

```{r, fig.height = 9, fig.width = 15}
df <- data.frame(colData(scp))
# df$Col <- factor(df$Col, levels = 1:24) ## TODO remove this line when added to scpdata
ggplot(df) +
    aes(x = Col,
        y = Row, 
        fill = log2(sum)) +
...
```{r}
set.seed(1234)
# sce$File.ID <- factor(sce$File.ID, levels = paste0("F", 1:192)) ## TODO Remove when updated in scpdata
sce <- runUMAP(sce, 
               ncomponents = 2,
               ntop = Inf,
               scale = FALSE,
...
All the clusters contain several cell type (except for cluster 2), and
all cell types are present in several clusters. So, this dataset, and 
...
each table provide the result of the differential expression analysis 
...
We next combine the list of tables into a single table. We perform 
some extra data wrangling to also include the smallest fold change 
...
that these stripes are artefacts of data processing (Vanderaa2021-xd). 
...
could indeed reproduce the cell filter performed by SCeptre.

```{r benchm_cell_filt, fig.width = 5, fig.height = 5, echo=FALSE, message = FALSE}
cells_sceptre <- paste0(sce_sceptre$`File ID`, sce_sceptre$Channel)
...
rows <- intersect(rownames(sce_sceptre),
                  rownames(sce))
cols <- intersect(paste0(sce_sceptre$`File ID`, sce_sceptre$Channel),
                  colnames(sce))
err <- assay(sce_sceptre)[rows, which(paste0(sce_sceptre$`File ID`, sce_sceptre$Channel) %in% cols)] -
    assay(sce)[rows, cols]
...
to accurately reproduce the analysis published in @Schoof2021-pv. It
...
### Running the `sceptre` module in R

In order to rerun this vignette, you will need to create a `conda` 
environment containing the required modules to run the SCeptre 
workflow. 

First, you need to install Python (we used 
[Python3](https://www.python.org/downloads/)) and
[conda](https://docs.conda.io/projects/conda/en/latest/user-guide/install/index.html)
(or [miniconda](https://docs.conda.io/en/latest/miniconda.html)).

Next, as mentioned in the 
[author's installation instructions](https://github.com/bfurtwa/SCeptre#installation-from-github),
create a new `conda` environment by running the following in a 
terminal.
...
Then you need to install `sceptre` and its dependencies. To do so, 
download the [GitHub repository](https://github.com/bfurtwa/SCeptre), 
and `cd` to the downloaded `SCeptre/` folder. Then run in the terminal: 

```{bash, eval = FALSE}
pip install .
```

To make use of sceptre within an R session, simply call (from the R 
terminal): 
