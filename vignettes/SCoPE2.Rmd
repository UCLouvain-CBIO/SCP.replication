...
single-cell applications and require new implementations. We 
develop a new data framework dedicated to SCP data analysis that 
combines two existing Bioconductor classes. The `SingleCellExperiment`
class provides an interface to many cutting edge methods for single-cell analysis and the `QFeatures` class facilitates manipulation and 
processing of MS-based quantitative data (blue boxes in the 
flowchart). The 
...
groups, are assigned to the protein group with most peptides (@Tyanova2016-yj,) and this can vary from one analysis to another. 
...
log-transformation and this is implemented in `logTransform` in the
`QFeatures`.
...
```{r, fig.width = 5, fig.height = 5, echo=FALSE, message = FALSE}
